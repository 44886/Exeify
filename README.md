...
- **自动取名**：应用名 / 文件名默认取入口网页的 `<title>`，不用手打，随时可改。
- **自定义图标 / 启动页 / 全屏**：exe 与 APK 通用；APK 还会自动套用应用名、图标、启动页、全屏，包名自动生成。
- **源码保护**（exe）：加密内嵌网页资源 + 禁用查看源码，防止产物被直接解压出源码。
- **登录状态持久化**：WebView2 用户数据固定保存到 `%LOCALAPPDATA%\exeify\<exe文件名>`，正常退出后 cookies、localStorage、IndexedDB 等可跨次启动保留；网站主动设置的会话 cookie 仍遵循网站自身的会话策略。
...
- 取页规则与 exe 一致：支持自定义入口页、前端路由（无扩展名路径）回退入口页，找不到只显示站内 404。
- 无原生标题栏，自动避让状态栏 / 导航栏；全屏模式则铺满屏幕。
- APK WebView 支持网页 `<input type="file">` 上传图片/视频、拍照/录像和 `getUserMedia()` 摄像头调用；首次使用时按 Android 系统提示授予相机/麦克风权限。
...
`--title 应用名`（不给则取网页 `<title>`） · `--entry 入口页` · `--icon 图标.ico/.png` · `--splash 启动图.png` · `--window fullscreen` · `--no-protect`。
